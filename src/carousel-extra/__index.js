	...
	edit: ( props ) => {
		...
		function EntriesList({categorias, cantidad}) {
			...
			if( posts !== null && posts?.length > 0 && posts?.length === posts?.length ){
				...
				//Comparar la extension del array.
				if( !attributes.SavePosts || ! attributes.SavePosts?.length > 0 || nuevoArray?.length != attributes.SavePosts?.length ){
					savethis(nuevoArray);
				}
			}
					...
					<ul>
						{ postsStored?.map( post => (
							<li key={ post.id }>
								...
								{ (post.thumbnail) ? <img src={'/image.png'} alt={ post.thumbnail } /> : null }
								{/* Traer extracto de cada entrada */}
								{decodeEntities( post.excerpt )}
							</li>
						)) }
					</ul>
					<code>{JSON.stringify(postsStored)}</code>
				</div>
			)
